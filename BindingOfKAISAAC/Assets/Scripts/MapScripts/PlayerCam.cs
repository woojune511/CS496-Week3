...
public class PlayerCam : MonoBehaviour{
    ...
    void LateUpdate(){
        playerPosition = player.GetComponent<Transform>().position;
        roomPosition = new Vector3((float)(Math.Floor(playerPosition.x/16)*16), (float)(Math.Floor(playerPosition.y/10)*10), 0);
        tr.position = roomPosition + new Vector3(7.5f, 4.5f, -10);
